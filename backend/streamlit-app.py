    ...
    if f is not None:
        ...
        if state.start:
            ...
            if state.run:
                ...
                model = attempt_load(f'/opt/ml/yolor/web/{ckpt_file}', map_location=device)
...
def ProcessFrames(vf, obj_detector, stop, confidence_threshold, width, height): 
    ...
    video_writer = cv2.VideoWriter(
                            "/opt/ml/yolor/web/result.mp4", fourcc, fps, (width, height)
    ...
    with st.spinner(text="Detecting Finished! Converting Video Codec..."):
        os.system("ffmpeg -i /opt/ml/yolor/web/result.mp4 -vcodec libx264 /opt/ml/yolor/web/result_h264.mp4")
    video_file = open("/opt/ml/yolor/web/result_h264.mp4", 'rb')
